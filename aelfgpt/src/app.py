...
from langchain_google_genai import GoogleGenerativeAI as SQLChatModel
...
if not all([llm_url, atlas_uri, llm_name, db_url, gemini_api_key]):
    logging.error("Environment variables AELFGPT_LLM_URL, AELFGPT_LLM_NAME, DB_URL, GEMINI_API_KEY, or ATLAS_URI are not set.")
...
@cl.on_chat_start
async def start():
    llm = Gemini(api_key=gemini_api_key, model="models/gemini-1.5-flash-latest")
    Settings.llm = llm
    ...
    llm = SQLChatModel(model="gemini-1.5-flash-latest", google_api_key=gemini_api_key)
...
async def main(message: cl.Message):
    ...
    if chat_profile == "THORIN":
        ...
        # await msg.send()
        res = gandalf.invoke(message.content)
        print(f"\n\nResponse: {res}\n\n")
        msg.send(res)